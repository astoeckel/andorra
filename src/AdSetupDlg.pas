...
type
  ...
  public
    function Execute(
      ...
      asections:TAdSetupDlgSections=[dlgResolution, dlgAdvancedOptions, dlgPlugin]; aform:TForm=nil):boolean;overload;
  end;
...
function TAdSetupFrm.Execute(
   acaption:string; acopyright:string; aimage:string; aaddraw:TAdDraw;
   asections:TAdSetupDlgSections; aform:TForm):boolean;
var
...
function TAdSetup.Execute(acaption, acopyright, aimage: string;
  aaddraw: TAdDraw; asections: TAdSetupDlgSections; aform: TForm): boolean;
var
  dlg:TAdSetupFrm;
begin
  dlg := TAdSetupFrm.Create(fowner);
  result := dlg.Execute(acaption, acopyright, aimage, aaddraw, asections, aform);
  dlg.Free;
end;

function TAdSetup.Execute: boolean;
begin
  result := Execute(FCaption, FCopyRight, FImage, FAdDraw, FSections, FForm);
end;
